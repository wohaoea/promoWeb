...
router.get(/^\/sdh5\S*$|^\/interest\S*$/, function(req, res, next) {
	/*活动时间*/
	var flag = 0
	if (new Date() > new Date('2016-1-19 00:00:00') || new Date() < new Date('2015-12-17 00:00:00')) {
		if (/^\/interesthelp\S*$/.test(req.path)) {
			if (new Date() > new Date('2016-1-19 00:30:00') || new Date() < new Date('2015-12-17 00:00:00')) {
...
router.get('/sdh5share/:shareRedId?', function(req, res, next) {
	...
	wsclient.getDoubleEggRedAmount(info, function(wsresult) {
		if (wsresult.success) {
...
router.get('/sdh5lq/:shareRedId?', function(req, res, next) {
	...
	wsclient.getDoubleEggRedList(info, function(wsresult) {
		if (wsresult.success) {
			res.locals.wxList = wsresult.data;
			wsclient.getDoubleEggRedShareHead(info, function(wsresult) {
				if (wsresult.success) {
...
router.get('/pla/sdh5lqlist', function(req, res, next) {
	wsclient.getDoubleEggRedList(req.query, function(wsresult) {
		if (wsresult.success) {
			res.locals.wxList = wsresult.data;
			res.locals.config = config;
			res.locals.tishiyu = tishiyu;
			res.locals.utils = utils;
			res.render('m6/shareredlist', res.locals);
		} else {
			//res.locals.shareperson = '';
			res.redirect(config.base.h5domain)
		}
...
router.get('/pla/getjxq', function(req, res, next) {
	...
	wsclient.getDoubleEggRedResult(req.query, function(wsresult) {
		if (wsresult.success) {
			if (wsresult.data == '-1') {
				res.send({
					success: false,
...
router.get("/smashingeggs", function(req, res, next) {
	...
	if (!req.session.user) {
		return res.render("m6/smashingeggs", {
			...
			_s_exchange: 0,
			_shareId: "",
			fudai:req.session.fudai,//取福袋session
			guoqing:req.session.guoqing
		});
	}
