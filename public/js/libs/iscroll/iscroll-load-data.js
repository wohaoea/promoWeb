(function(window){
function IScrollLoadData(wrapperEl,contentEl,dropTopAction,dropBottomAction,limit,count){
	...
	this.pullUpLabel_Text='<div class="loading-box"></div>';
	this.pullDownLabel_Text='<div class="loading-box"></div>';
...
IScrollLoadData.prototype.createIScroll=function(){
	//var count =window.countPage;
	var self=this;
	this.myScroll = new IScrollLoadData.IScroll(this.wrapper, {
		probeType: 2, mouseWheel: false,bindToWrapper:true,scrollY:true,tap:true,click:true
	}).on('scroll',function(){
		
		if(window.countPage>=self.limit){
			return
		}
		if (this.y > 50 &&  
