...
$(function() {
	...
	var layerTip = function(flag) {
		...
		//creattip(true);
		$('.boxfixed,.shareTip').bind('touchend', function(event) {
	...
	var layer = function(opt) {
		...
		opt.bindEvent = (function() {
			$('.layerClose').bind('touchend', function() {
				$('.layerwrap,.boxfixed').remove();
			})
		})();
		opt.callback();
...
*/
	$(".btnlq").bind('touchend', function(event) {
			$.layer({
				html: '<h3>填写手机号，绑定现金红包</h3><div class="inputbox"><input type="tel" placeholder="输入手机号" id="mephonenum"/><div class="errmsg"></div></div><div class="tijiao"></div><div class="tijiaoz"></div>',
				callback: function() {
					var _this = this;
					$("#mephonenum").focus();
					$('.errmsg').text('');
					$('.boxfixed').bind('touchend', function(event) {
					...
					$(".tijiao").bind('touchend', function(event) {
								...
								$.ajax({
									...
									success: function(result) {
										...
										if (result.success) {
											if (result.telVali == '0') {
												$.layer({
													html: '<h3>恭喜您获得<span>' + result.amount + '元</span>现金券</h3><p>现金券已发放至您的账户</p><p>有效期至：' + result.endtime + '</p><a class="ljzcbtn" href="' + result.domh5 + 'user/reg?sid=1117&mob=' + mobile + '"><img src="/imgs/sdhd/ljzc.png"/></a>',
													lock: false,
													callback: function() {
														$.get('/m6/pla/sdh5lqlist',myinfo, function(result) {
											...
											if (result.telVali == '1') {
												$.layer({
													html: '<h3>恭喜您获得<span>' + result.amount + '元</span>现金券</h3><p>&nbsp;</p><p>现金券已发放至您的账户</p><a class="chakanxjq" href="' + result.domh5 + 'user/login?tourl=/account/redpacket"><img src="/imgs/sdhd/chakanxjq.png"/></a>',
													lock: false,
													callback: function() {
														$.get('/m6/pla/sdh5lqlist', myinfo, function(result) {
															$(".liebiaowrap>ul").empty().append(result);
		...
		/*微信弹出分享提示*/
	$(".btnshare,.vedbtnlq").bind('touchend', function(event) {
		$.layerTip();
	});
	...
	$('.vbtnlq').bind('touchend', function(event) {
		var that = $(this);
		/*$.layer({
		...
		$.layer({
			html: '<h3>恭喜您获得<span>' + that.data('amount') + '元</span>现金券</h3><p>现金券已发放至您的账户</p><p>有效期至：' + that.data('overtime') + '</p><a class="ljzcbtn" href="' + h5url + 'user/reg?sid=1117&mob=' + that.data('tel') + '"><img src="/imgs/sdhd/ljzc.png"/></a>',
			callback: function() {
...
$(function() {
	...
	$.timeDown(arr, function(mydom) {
		...
		var myrecordNo = mydom.closest("li").data('recordno');
		//alert(myrecordNo);
		$.ajax({
			type: "GET",
			url: "/m6/pla/hqjxz/" + myrecordNo,
			dataType: "json",
			data: {},
			beforeSend: function() {},
			complete: function() {},
			...
			success: function(result) {
				if (result.success) {
					mydom.closest("li").find('.addrated').text(result.data.addRate);
				}
	...
	function getTimeDown() {
		if (typeof(ishare) != 'undefined') {
			if (ishare == 1) {
				...
				$(".jxbtnshare").bind('touchend', function(event) {
					$.layerTip()
				});
